...
export class ExportService {
  async generateInvoiceExport(
    ...
    const totalExclusionHours = exclusionDetails.reduce(
      (sum, d) => sum + d.hours,
      0,
    );
    const totalExclusionCost = exclusionDetails.reduce(
      (sum, d) => sum + d.cost,
      0,
    );
    ...
    const coveredBySubscription =
      coverageResult.rows[0]?.has_subscription_coverage || false;
    ...
    const totalBillable = billableLines.reduce(
...
`;
  ...
  async getBudgetVsBurn(projectId: string, month: Date): Promise<any> {
    ...
    const hoursUtilization = data.budget_hours
      ? (parseFloat(data.actual_hours) / parseFloat(data.budget_hours)) * 100
      : 0;
    const costUtilization = data.budget
      ? (parseFloat(data.actual_cost) / parseFloat(data.budget)) * 100
      : 0;
