...
export function HubSpotDashboardEnhanced() {
  ...
  const [usingImportedDeals, setUsingImportedDeals] = useState(false);
  ...
  // Fetch deals - try API first, fallback to uploaded deals
  const fetchDeals = async () => {
    setLoading(true);
    setError(null);
    setUsingImportedDeals(false);
      ...
      if (!response.ok) {
        ...
        if (uploadResponse.ok) {
          ...
          if (uploadData.success && uploadData.deals.length > 0) {
            // Transform uploaded deals to match HubSpot format
            const transformedDeals = uploadData.deals.map((deal: any, index: number) => ({
              id: deal.id || `uploaded_${index}`,
              properties: {
                dealname: deal.name,
                ...
                createdate: deal.createDate || new Date().toISOString(),
              },
            }));
            setDeals(transformedDeals);
            setUsingImportedDeals(true);
            return;
          }
        }
        const errorData = await response.json();
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {/* Status Messages */}
      {usingImportedDeals && activeTab === "deals" && (
        <div className="mb-4 p-4 bg-green-100 border border-green-400 text-green-700 rounded">
          Showing imported deals from Excel/CSV. The HubSpot API is not configured or unavailable.
        </div>
      )}
        ...
        <div className="mb-4 p-4 bg-red-100 border border-red-400 text-red-700 rounded">
          ...
          {error.includes("not configured") && (
            <div className="mt-2 text-sm">
              ...
              <br />
              <br />
              Or <Link href="/hubspot/upload" className="text-blue-700 underline">import deals from Excel/CSV</Link> instead.
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === "deals" && !loading && deals.length === 0 && !error && (
        <div className="text-center py-8 text-gray-500">
          <p className="mb-4">No deals found. Click refresh to load deals.</p>
          <Link
            href="/hubspot/upload"
            className="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700"
          >
